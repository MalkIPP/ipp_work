...
def simulation(year, ind_variables, foy_variables):    
    # couple
    ...
    simulation_couple = simulation_couple.new_simulation(debug = True, reference = True)
    ...
    simulation_couple_enfant = simulation_couple_enfant.new_simulation(debug = True, reference = True)
    ...
    simulation_celib = simulation_celib.new_simulation(debug = True, reference = True)
    ...
    simulation_celib_enfant = simulation_celib_enfant.new_simulation(debug = True, reference = True)
...
def df_with_tax_rate(df):
    target = df['irpp'].values
    varying = df['salaire_imposable'].values
    from openfisca_core.rates import average_rate, marginal_rate
    marginal_rate = marginal_rate(target, varying)
    average_rate = average_rate(target, varying)
    average_rate = pandas.Series(average_rate ,index=range(0, average_rate.shape[0]), )[1:]
    marginal_rate = pandas.Series(marginal_rate, index=range(0, marginal_rate.shape[0]))
    
    df2=pandas.DataFrame({'marginal_rate': marginal_rate,
...
def wrap_up(year):
    ...
    foy_variables = ['irpp']
    df_by_type_menage = simulation(year, ind_variables, foy_variables)
